...
mean_ozone = mean(data_ozone)
std_ozone = stdev(data_ozone)

data_solar_r = [z[1] for z in data_airquality]
mean_solar_r = mean(data_solar_r)
std_solar_r = stdev(data_solar_r)

data_wind = [z[2] for z in data_airquality]
mean_wind = mean(data_wind)
std_wind = stdev(data_wind)

data_temp = [z[3] for z in data_airquality]
mean_temp = mean(data_temp)
std_temp = stdev(data_temp)


data_ozone = [(z[0]-mean_ozone)/std_ozone for z in data_airquality]
data_solar_r = [(z[1]-mean_solar_r)/std_solar_r for z in data_airquality]
data_wind = [(z[2]-mean_wind)/std_wind for z in data_airquality]
data_temp = [(z[3]-mean_temp)/std_temp for z in data_airquality]

data_normalized_airquality = [(a,b,c,d) for a,b,c,d in zip(data_ozone, data_solar_r, data_wind, data_temp)]
...
class HW4DensityReg(MCMC_Gibbs):
    def __init__(self, initial, yx_data, N_truncation):
        ...
        #hyperparams
        self.hyper_v0 = 4000
        self.hyper_Lambda0 = np.eye(4)*0.01
...
if __name__=="__main__":
    ...
    diag_inst = MCMC_Diag()
    if run:
        initial = [[(data_normalized_airquality[i], np.eye(4)) for i in range(trunc_N)], 
                [1/trunc_N for _ in range(trunc_N)], 
                [i%trunc_N for i in range(len(data_normalized_airquality))], 
                [0,0,0,0],
                np.eye(4),
                10]
        gibbs_inst = HW4DensityReg(initial, data_normalized_airquality, trunc_N)
    ...
    diag_inst_L.set_variable_names(['L'+str(i) for i in range(len(data_normalized_airquality))])
    ...
    ###########################
    # Inference
    infer_inst = HW4DensityReg_Infer(data_normalized_airquality, diag_inst.MC_sample)
    # "1",41, 190, 7.4, 67, 5, 1
    # "2",36, 118, 8, 72, 5, 2
    ...
    # ozone vs solar_R
    solar_r_grid = np.linspace(-2, 2, 50)
    y_vs_solar_r_lower = []
    y_vs_solar_r_mean = []
    y_vs_solar_r_upper = []
    for solar_r in solar_r_grid:
        quant = infer_inst.y_given_x_G_mean_quantile(np.array([solar_r, 0, 0]))
    ...
    # ozone vs Wind
    wind_grid = np.linspace(-2, 2, 50)
    y_vs_wind_lower = []
    y_vs_wind_mean = []
    y_vs_wind_upper = []
    for wind in wind_grid:
        quant = infer_inst.y_given_x_G_mean_quantile(np.array([0, wind, 0]))
    ...
    # ozone vs Temp
    temp_grid = np.linspace(-2, 2, 50)
    y_vs_temp_lower = []
    y_vs_temp_mean = []
    y_vs_temp_upper = []
    for temp in temp_grid:
        quant = infer_inst.y_given_x_G_mean_quantile(np.array([0, 0, temp]))
